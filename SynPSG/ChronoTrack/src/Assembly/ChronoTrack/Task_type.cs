...
/// </summary>
	[XFAttr(size=80)]
	[Serializable]
	public class Task_type : IStructWire
	{
		/// <summary>
		...
		static Task_type()
		{
			...
			fieldAttributes[3]= new XFAttr(XFAttr.xftype.ALPHA,20);
		}
		...
		{
			get{ return f_Task_type_id;}
			set { 
			    if(f_Task_type_id != value) 
			       m_changed = true; 
			    f_Task_type_id = value;
			} 
		}
		...
		/// structure clone
		/// </summary>
		public Task_type Clone()
		{
			Task_type tmp = new Task_type();
			tmp.f_Synergygrfa = this.f_Synergygrfa;
			tmp.f_Task_type_id = this.f_Task_type_id;
		...
		/// structure equals
		/// </summary>
		public bool Equals(Task_type str)
		{
			bool eq = true;
			...
			if (this.Task_type_id != str.Task_type_id)
			{
				eq = false;
				return eq;
			}
		...
		public string structToWire()
		{
			...
			body.Append(XFProtocol.fieldToWire(f_Task_type_id,fieldAttributes[1]));
			body.Append(XFProtocol.fieldToWire(f_Description,fieldAttributes[2]));
			body.Append(XFProtocol.fieldToWire(f_Replication_key,fieldAttributes[3]));
			body.Append("^");
		...
		public void wireToStruct(SynMessage reply)
		{
			...
			f_Task_type_id = XFProtocol.wireToint(reply);
		...
		/// </summary>
		public static void GetRow(ref Task_type str, int pos, DataTable dt)
		{
			DataRow dr = dt.Rows[pos];
			str.f_Synergygrfa = (string)dr["Synergygrfa"];
			str.f_Task_type_id = (int)dr["Task_type_id"];
		...
		public static void TableHeader(ref DataTable dt)
		{
			...
			dtclm = new DataColumn("Task_type_id",typeof(int));
		...
		public static void FillDataTable(ArrayList al, ref DataTable dt, string type, int pos)
		{
			...
			{
				dr = dt.NewRow();
				Task_type str = (Task_type)al[i];
				dr["Synergygrfa"] = str.Synergygrfa;
				dr["Task_type_id"] = str.Task_type_id;
